...
from newsapi import NewsApiClient # <<<--- ADDED FOR NEWSAPI.ORG
...
@st.cache_data
def fetch_enriched_news(ticker: str, ticker_info_data: dict) -> list[dict]: # Added ticker_info_data
    """
    ...
    try:
        ...
        for news_item in raw_news:
            ...
            enriched_item['source_api'] = 'Yahoo Finance' # Tag source
        ...
        return [{"error": f"Failed to fetch/enrich Yahoo Finance news for {ticker}: {e}"}]
        ...
        if all_articles_response.get("status") == "ok" and "articles" in all_articles_response:
            for article in all_articles_response["articles"]:
                ...
                if article.get('publishedAt'):
                    ...
                    except ValueError: pass # Handle potential parsing errors
...
def fetch_insider_filings(ticker: str) -> list[dict]:
    try:
        ...
        for r in recs:
            ...
            if "purchase" in tx or "buy" in tx: r["type"] = "buy"
            elif "sale" in tx or "sell" in tx: r["type"] = "sell"
            else: r["type"] = "other"
        return recs
    except Exception as e:
        return []
...
def fetch_fair_value_from_value_trades(ticker: str, company_name: str) -> dict:
    ...
    Effective scraping requires significant effort to replicate browser behavior.
    """
    error_message = (
...
def fetch_politician_trades(ticker: str, days_back: int = 365) -> list[dict]:
    ...
    try:
        ...
        for row_link_tag in trade_rows[:20]:
            cells = row_link_tag.find_all('div', recursive=False) # Get direct children divs
            # This parsing logic is highly dependent on CapitolTrades structure and may need frequent updates.
            # It assumes a certain order or class names that might not be stable.
            # A more robust scraper would use more specific and resilient selectors.
            try:
                politician_name = row_link_tag.select_one("div[class*='politician-name'], div[data-field='politician.name']") 
                tx_type = row_link_tag.select_one("div[class*='tx-type'], div[data-field='txType']")
                value_range = row_link_tag.select_one("div[class*='tx-value'], div[data-field='valueFormatted']") 
                date_str = row_link_tag.select_one("div[class*='tx-date'], div[data-field='txDateFormatted']") 

                if all([politician_name, tx_type, value_range, date_str]):
                    name = politician_name.text.strip()
                    tx_type_text = tx_type.text.strip().lower()
                    transaction = "purchase" if "purchase" in tx_type_text or "buy" in tx_type_text else \
                                  "sale" if "sale" in tx_type_text or "sell" in tx_type_text else "other"
                    val_rng_text = value_range.text.strip()
                    date_text = date_tag.text.strip()
                    value_est = 0
                    val_matches = re.findall(r'\<span class="math-inline">\(\[\\d,\]\+\)', val\_rng\_text\)
if val\_matches\: value\_est \= int\(val\_matches\[0\]\.replace\(',', ''\)\)
politician\_trades\_list\.append\(\{
"politician\_name"\: name, "transaction\_type"\: transaction,
"value\_range"\: val\_rng\_text, "value\_estimate\_lower"\: value\_est,
"date\_str"\: date\_text, "source\_url"\: "https\://www\.capitoltrades\.com" \+ row\_link\_tag\['href'\]
\}\)
except Exception\: \# Skip row if parsing fails for that specific row
continue 
if not politician\_trades\_list and trade\_rows\:
return \[\{"error"\: f"CT\: Found trade rows for \{ticker\} but failed to parse individual fields\. Selectors may need update\."\}\]
return politician\_trades\_list
except requests\.exceptions\.Timeout\: return \[\{"error"\: f"CT\: Timeout for \{ticker\}"\}\]
except requests\.exceptions\.HTTPError as e\: return \[\{"error"\: f"CT HTTP error for \{ticker\}\: \{e\}"\}\]
...
self\.api\_key \= api\_key; self\.provider \= provider
OPENAI\_DEFAULT\_MODEL \= "gpt\-4o"; DEEPSEEK\_DEFAULT\_MODEL \= "deepseek\-reasoner"
...
\# \.\.\. \(embedding logic \- can be simplified if not used currently\)
try\:
resp \= self\.client\.embeddings\.create\(input\=texts, model\=model\_id\)
return \[e\.embedding for e in resp\.data\]
except Exception as e\: raise Exception\(f"Embedding Error\: \{e\}"\)
def generate\(self, prompt\: str\) \-\> str\:
try\:
stream \= self\.client\.chat\.completions\.create\(model\=self\.model\_name, messages\=\[\{"role"\: "user", "content"\: prompt\}\], stream\=True\)
final\_content \= ""\.join\(chunk\.choices\[0\]\.delta\.content for chunk in stream if chunk\.choices and chunk\.choices\[0\]\.delta and chunk\.choices\[0\]\.delta\.content\)
...
m1  \= \(P\_t/P\_1m\)\-1  if pd\.notna\(P\_1m\) and P\_1m \!\= 0 else 0\.0
m12 \= \(P\_t/P\_12m\)\-1 if pd\.notna\(P\_12m\) and P\_12m \!\= 0 else 0\.0; signal \= "hold"
...
news\_items\_from\_bundle \= data\.get\("news", \[\]\)
news\_status\_error \= data\.get\("news\_fetch\_status\_error"\) \# Check if orchestrator passed an error status
if news\_status\_error\: \# If there was a general news fetching error
return \{"ticker"\: ticker, "sentiment\_score"\: 0\.0, "sentiment\_signal"\: "hold", "sentiment\_error"\: news\_status\_error\}
if not news\_items\_from\_bundle or \(isinstance\(news\_items\_from\_bundle, list\) and len\(news\_items\_from\_bundle\) \> 0 and isinstance\(news\_items\_from\_bundle\[0\], dict\) and "error" in news\_items\_from\_bundle\[0\]\)\:
return \{"ticker"\: ticker, "sentiment\_score"\: 0\.0, "sentiment\_signal"\: "hold",
"sentiment\_error"\: news\_items\_from\_bundle\[0\]\.get\("error"\) if news\_items\_from\_bundle and isinstance\(news\_items\_from\_bundle, list\) and news\_items\_from\_bundle\[0\] else "No news items found for sentiment\."\}
content\_for\_llm \= \[\]
company\_name\_overall \= data\.get\("ticker\_info",\{\}\)\.get\('longName', ticker\)
for item in news\_items\_from\_bundle\[\:7\]\:
if isinstance\(item, dict\) and "error" not in item\:
title \= item\.get\('title', ''\)
publisher \= item\.get\('publisher', ''\)
description \= item\.get\('description', ''\)
text\_snippet \= f"Headline\: \{title\}"
if description and isinstance\(description, str\)\: text\_snippet \+\= f" \| Snippet\: \{description\[\:200\]\}\.\.\."
if publisher and publisher \!\= 'N/A'\: text\_snippet \+\= f" \(Source\: \{publisher\} via \{item\.get\('source\_api', 'Unknown'\)\}\)"
content\_for\_llm\.append\(text\_snippet\)
if not content\_for\_llm\:
return \{"ticker"\: ticker, "sentiment\_score"\: 0\.0, "sentiment\_signal"\: "hold", "sentiment\_error"\: "No processable news content for LLM\."\}
prompt \= \(f"Analyze sentiment for \{company\_name\_overall\} \(\{ticker\}\) based on news from past month\. Score \-1\.0 \(v\.neg\) to \+1\.0 \(v\.pos\)\. Output only number\.\\n\\nNews\:\\n" \+ "\\n\\n"\.join\(f"\- \{c\}" for c in content\_for\_llm\)\)
...
sig \= "buy" if score \> 0\.25 and not llm\_error\_msg else \("sell" if score < \-0\.25 and not llm\_error\_msg else "hold"\)
return \{"ticker"\:ticker, "sentiment\_score"\:score, "sentiment\_signal"\:sig, "sentiment\_error"\: llm\_error\_msg\}
...
def run\(self, ticker\: str, data\: dict\) \-\> dict\:
ticker\_info\_res \= data\.get\("ticker\_info", \{\}\); price\_history\_df \= data\.get\("price\_history"\) \# Use different var name to avoid conflict
current\_price \= ticker\_info\_res\.get\("currentPrice"\) or \(price\_history\_df\["Close"\]\.iloc\[\-1\] if price\_history\_df is not None and not price\_history\_df\.empty else None\)
target\_mean\_price \= ticker\_info\_res\.get\("targetMeanPrice"\); recommendation \= str\(ticker\_info\_res\.get\("recommendationKey", "hold"\)\)\.lower\(\); upside \= 0\.0
...
if error\:
is\_ignorable\_error \= \(error \=\= error\_fv\_not\_found\_page or
...
if not is\_ignorable\_error\: significant\_error \= True
...
company\_name\_for\_news \= ticker\_info\.get\('longName', ticker\_info\.get\('shortName', t\)\)
combined\_news\_data\_list \= \[\]
news\_fetch\_status\_messages \= \[\]
if configs\["use\_sentiment"\]\:
yfinance\_news \= fetch\_enriched\_news\(t, ticker\_info\) \# Pass ticker\_info here
...
elif yfinance\_news and isinstance\(yfinance\_news\[0\], dict\) and "error" in yfinance\_news\[0\]\:
news\_fetch\_status\_messages\.append\(f"Yahoo News Error\: \{yfinance\_news\[0\]\['error'\]\}"\)
if llm\_client and st\.secrets\.get\("NEWSAPI\_KEY"\)\:
newsapi\_articles \= fetch\_comprehensive\_news\_from\_api\(t, company\_name\_for\_news, lookback\_days\=30\)
if newsapi\_articles and not \(isinstance\(newsapi\_articles\[0\], dict\) and "error" in newsapi\_articles\[0\]\)\:
...
news\_fetch\_status\_messages\.append\(f"NewsAPI Error\: \{newsapi\_articles\[0\]\['error'\]\}"\)
elif configs\["use\_sentiment"\]\: \# if sentiment is on but no NewsAPI key
...
if not deduplicated\_news and not news\_fetch\_status\_messages\: \# No news and no errors means simply no news found
...
"news"\: deduplicated\_news, 
"news\_fetch\_status\_error"\: news\_fetch\_status\_for\_bundle if "Error" in news\_fetch\_status\_for\_bundle else None, \# Pass error specifically
"insider\_filings"\: fetch\_insider\_filings\(t\) if configs\["use\_filings"\] else \[\],
"politician\_trades"\: politician\_trades\_list,
"value\_trades\_fair\_value\_data"\: fetch\_fair\_value\_from\_value\_trades\(t, company\_name\_for\_news\) if configs\["use\_value\_trades"\] else \\
...
"news\_headlines\_for\_popover"\: \[
f"\{n\.get\('publish\_time\_readable','N/A'\)\} \- \{n\.get\('title', 'N/A'\)\} \(\{n\.get\('publisher','N/A'\)\} via \{n\.get\('source\_api','Unknown'\)\}\)" 
for n in deduplicated\_news\[\:10\] \# Use the combined, sorted, de\-duplicated list
\],
"politician\_trades\_for\_popover"\: \[pt for pt in politician\_trades\_list\[\:5\] if isinstance\(pt, dict\) and "error" not in pt\],
"news\_status\_display"\: news\_fetch\_status\_for\_bundle 
\}
...
val\_s \= \{"Forward P/E"\: f"\{res\.get\('forward\_pe',0\)\:\.1f\}", "Relative P/E Signal"\: res\.get\('relative\_pe\_signal', "N/A"\)\.upper\(\), "DCF Fair Price \(Simple Est\.\)"\: f"</span>{res.get('dcf_fair_price',0):.2f}" if res.get('dcf_fair_price') is not None else "N/A", "DCF Signal": res.get('dcf_signal', "N/A").upper()}
                        ...
                        if live_configs_main["use_value_trades"]:
                            ...
                            if res.get('vt_fair_value_estimate') is None and not res.get('vt_data_error'):
                                ...
                                vt_scrape_status = "Not Attempted / Config Error"
...
"VT Site Current Price"\: f"</span>{res.get('vt_site_current_price'):.2f}" if res.get('vt_site_current_price') is not None else "N/A",
                    ...
                    with tabs[3]: # News & Filings Tab
                        if live_configs_main["use_sentiment"]:
                            ...
                            llm_status_message = res.get("news_status_display", "Status: OK") # Get news fetch status
                            if sent_error : llm_status_message += f" | LLM: {sent_error}" # Append LLM specific error
                            ...
                            if news_headlines:
                                with st.popover("View News Headlines"):
                                    for title in news_headlines:
                                        st.markdown(f"- {title}")
                            elif "Error" not in llm_status_message and "No news items" not in llm_status_message :
                                 st.caption("No news headlines processed for display.")
