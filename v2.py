...
def fetch_inst_filings(ticker: str) -> list[dict]:
    try:
        ...
        if df is not None: return df.to_dict("records")
        return []
...
overall\_news\_fetch\_error \= data\.get\("news\_fetch\_status\_error"\)
if overall\_news\_fetch\_error\:
...
for item in valid\_news\_items\[\:7\]\:
...
return \{"ticker"\:ticker, "sentiment\_score"\:score, "sentiment\_signal"\:sig, "sentiment\_error"\: final\_error\_message\_for\_sentiment\}
...
def run\(self, ticker\: str, data\: dict\) \-\> dict\:
ticker\_info\_res \= data\.get\("ticker\_info", \{\}\); price\_history\_df \= data\.get\("price\_history"\)
...
if error\:
is\_ignorable\_error \= \(
error \=\= error\_fv\_not\_found\_page or
...
news\_fetch\_status\_messages\.append\(f"Yahoo News\: \{yfinance\_news\[0\]\['error'\]\}"\)
...
"news\_fetch\_status\_error"\: news\_fetch\_status\_for\_bundle if "Error" in news\_fetch\_status\_for\_bundle or "failed" in news\_fetch\_status\_for\_bundle\.lower\(\) or "No news" in news\_fetch\_status\_for\_bundle else None,
...
for n in deduplicated\_news\[\:10\]
\],
...
use\_sentiment\_live\_main \= st\.checkbox\("News Sentiment \(LLM\)", value\=True if llm\_client else False, disabled\=not llm\_client, key\="live\_sentiment\_cb\_main", help\="Uses LLM for news sentiment\. Requires NewsAPI key if used, else Yahoo Finance\."\)
