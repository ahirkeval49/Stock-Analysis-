...
class ModelClient:
    ...
    def generate(self, prompt: str) -> str:
        try:
            ...
            return f"Error: Could not generate LLM response."
...
class VolatilityAgent:
    def run(self, ticker: str, data: dict, price_data_slice: pd.DataFrame = None) -> dict:
        beta = data.get("ticker_info", {}).get("beta", 1.0); sig  = "sell" if beta > 1.5 else ("buy" if beta < 0.8 else "hold")
...
class SentimentAgent:
    ...
    def run(self, ticker: str, data: dict) -> dict:
        ...
        if not headlines: return {"ticker":ticker, "sentiment_score":0.0, "sentiment_signal":"hold"}
        prompt = (f"Rate sentiment for {ticker} (−1 negative, +1 positive) based ONLY on:\n" + "\n".join(f"- {h}" for h in headlines) + "\n\nOutput only the number.")
        try: response = self.client.generate(prompt).strip(); score = float(response)
        except: score = 0.0
    ...
    def run(self, ticker: str, data: dict) -> dict:
        ...
        if insiders:
            for r in insiders:
                ...
                try:
                    # Attempt to convert to int after removing commas
                    shares = int(str(shares_val).replace(',',''))
                except ValueError:
                    shares = 0 # If conversion fails, assume 0 shares
...
class PoliticianFilingsAgent:
    def run(self, ticker: str, data: dict) -> dict:
        trades = data.get("politician_trades", []); net_value_estimate = 0; buy_count = 0; sell_count = 0
        if trades:
            for trade in trades:
        ...
        return {"ticker": ticker, "politician_net_trade_value_estimate": net_value_estimate,
                "politician_buy_tx_count": buy_count, "politician_sell_tx_count": sell_count, "politician_filings_signal": signal}

class FairValueAgentVT:
    def run(self, ticker: str, data: dict) -> dict:
        vt_data = data.get("value_trades_fair_value_data", {}); fair_value = vt_data.get("vt_fair_value"); error = vt_data.get("error")
        current_price_data = data.get("ticker_info", {})
        current_price = current_price_data.get("currentPrice")
        if current_price is None and data.get("price_history") is not None and not data["price_history"].empty: # Fallback to last close
        ...
        if error and error not in ["FV not found on page.", "VT Configuration incomplete in secrets.", "VT: Skipped by user."]:
             # Error would have been shown by the fetcher via st.error or st.warning
             pass
...
class PortfolioAgent:
    ...
    def run(self, ticker: str, signals: list[dict], agent_weights: dict = None) -> dict:
        ...
        agg_signals = {}; [agg_signals.update(s_dict) for s_dict in signals]
...
def run_live_analysis(tickers, history_years, llm_client, configs):
    ...
    for t in tickers:
        ...
        if price_history_full.empty: results[t] = {"error": "Failed to fetch price history", "ticker": t, "final_decision":"error", "composite_score":0}; continue
        ticker_info = fetch_ticker_info(t)
        if not ticker_info: results[t] = {"error": "Failed to fetch ticker info", "ticker": t, "final_decision":"error", "composite_score":0}; continue
        ...
        for agent_instance in all_agents_instances:
            ...
            except Exception as e: agent_results_list.append({agent_name.lower().replace("agent","") + "_signal": "error", "error_details": str(e)})
        ...
        current_result_dict = {"ticker": t, "current_price_display": current_price_for_ticker,
                               ...
                               "news_headlines_for_display": [n.get('title') for n in news_data_list[:5]], # Add news for display
                               "politician_trades_for_display": politician_trades_list[:5] # Add politician trades for display
                               }
        for res_dict in agent_results_list: current_result_dict.update(res_dict)
...
# LLM Client Initialization (remains in sidebar for status)
llm_client = None
try:
...
with config_container:
    app_mode = st.radio("Select Mode:", ["Live Analysis", "Backtesting"], key="app_mode_select_main", horizontal=True)
    st.markdown("---")

    if app_mode == "Live Analysis":
        st.subheader("Live Analysis Settings")
        tickers_in = st.text_input("Tickers (comma-separated):", "AAPL,MSFT,GOOG", key="live_tickers_input_main")
        history_years_live = st.slider("Historical Data for Analysis (Years):", 1, 10, 5, key="live_history_slider_main")

        st.subheader("Feature Toggles (Live Analysis)")
        cols_features = st.columns(3)
        with cols_features[0]:
            use_sentiment_live = st.checkbox("News Sentiment (LLM)", value=True if llm_client else False, disabled=not llm_client, key="live_sentiment_cb_main", help="Uses LLM for news sentiment. Requires API key.")
            use_filings_live = st.checkbox("Insider Filings", value=True, key="live_insider_cb_main", help="Analyzes yfinance insider transaction data.")
        with cols_features[1]:
            use_politician_filings_live = st.checkbox("Politician Filings", value=False, key="live_politician_cb_main", help="EXPERIMENTAL: Attempts to scrape CapitolTrades.com. May be slow/unreliable.")
            use_value_trades_live = st.checkbox("Value-Trades Fair Value", value=False, key="live_vt_cb_main", help="EXPERIMENTAL: Attempts login to Value-Trades.com. Requires VT_ secrets. CHECK ToS!")

        st.markdown("")
        run_button_live = st.button("🚀 Run Live Analysis", use_container_width=True, type="primary", key="run_live_btn_main")

    elif app_mode == "Backtesting":
        st.subheader("Backtesting Settings")
        bt_ticker = st.text_input("Ticker for Backtest:", "AAPL", key="bt_ticker_input_main").upper()
        ...
        with col1_bt:
            ...
            bt_start_date = st.date_input("Start Date:", default_bt_start_date, max_value=default_bt_end_date - timedelta(days=1), key="bt_start_date_main").strftime("%Y-%m-%d")
        with col2_bt:
            bt_end_date = st.date_input("End Date:", default_bt_end_date, min_value=datetime.strptime(bt_start_date, "%Y-%m-%d") + timedelta(days=1), key="bt_end_date_main").strftime("%Y-%m-%d")

        bt_initial_capital = st.number_input("Initial Capital:", 1000, 1000000, 10000, 1000, key="bt_capital_input_main", format="%d")
        ...
        with st.expander("Adjust Backtest Agent Weights (Simplified Strategy)", expanded=False):
            ...
            bt_weights_price = st.slider("Price Signal Weight:", 0.0, 2.0, 1.0, 0.1, key="bt_w_price_main")
            bt_weights_momentum = st.slider("Momentum Signal Weight:", 0.0, 2.0, 0.8, 0.1, key="bt_w_momentum_main")
            bt_weights_volatility = st.slider("Volatility Signal Weight:", 0.0, 2.0, 0.2, 0.1, key="bt_w_vol_main")

        backtest_portfolio_weights = {
            "price": bt_weights_price, "momentum": bt_weights_momentum, "volatility": bt_weights_volatility,
        ...
        st.markdown("")
        run_button_backtest = st.button("📈 Run Backtest", use_container_width=True, type="primary", key="run_bt_btn_main")

# Main App Logic (Execution and Display of Results)
st.markdown("---")

# Check if the app_mode variable is defined which happens after the radio button is interacted with
if 'app_mode' in locals() or 'app_mode' in st.session_state : # Use st.session_state if persisting mode
    current_app_mode = locals().get('app_mode') or st.session_state.get('app_mode') # Get mode

    if current_app_mode == "Live Analysis":
        # Ensure run_button_live is defined from its block
        if 'run_button_live' in locals() and run_button_live and 'tickers_in' in locals() and tickers_in:
            ...
            if not live_tickers_list:
                ...
                live_configs = {"use_sentiment": use_sentiment_live, "use_filings": use_filings_live,
                                ...
                                "use_value_trades": use_value_trades_live}

                if 'live_output' not in st.session_state:
                    st.session_state.live_output = {}

                with st.spinner("⏳ Running AI agents for live analysis... This may take a moment, especially with experimental features."):
                    st.session_state.live_output = run_live_analysis(live_tickers_list, history_years_live, llm_client, live_configs)
                ...
                for i in range(0, num_tickers, cols_per_row):
                    ...
                    for idx, t_symbol in enumerate(row_tickers):
                        with cols[idx]:
                            ...
                            card_color_map = {"BUY": "green", "SELL": "red", "HOLD": "#FFA500"}
                ...
                for t_symbol in live_tickers_list:
                    ...
                    with st.expander(f"🔍 Detailed Analysis for {t_symbol}"):
                        ...
                        with tabs[0]:
                            ...
                            price_history_df_display_exp = fetch_price_history(t_symbol, period=f"{history_years_live}y")
                            ...
                            if live_configs["use_sentiment"]:
                                ...
                                if res.get("news_headlines_for_display"):
                                   with st.popover("View News Headlines"):
                                       for news_item_title in res["news_headlines_for_display"][:5]: st.markdown(f"- {news_item_title}")
                            ...
                            if live_configs["use_politician_filings"]:
                                ...
                                if res.get("politician_trades_for_display"):
                                   with st.popover("View Scraped Politician Trades (Max 5)"):
                                       for p_trade in res["politician_trades_for_display"][:5]:
    ...
    elif current_app_mode == "Backtesting":
        if 'run_button_backtest' in locals() and run_button_backtest and 'bt_ticker' in locals() and bt_ticker:
            ...
            with st.spinner(f"⏳ Running backtest for {bt_ticker}... This may take a while."):
                st.session_state.bt_metrics, st.session_state.bt_log_df = run_backtest(
                    bt_ticker, bt_start_date, bt_end_date,
                    bt_initial_capital, llm_client, backtest_portfolio_weights
                )

            if st.session_state.bt_metrics and "message" not in st.session_state.bt_metrics :
                ...
                if not st.session_state.bt_log_df.empty:
                    ...
                    with st.expander("View Backtest Log and Signals (Last 1000 rows)"):
                ...
                st.error(f"Backtest failed: {st.session_state.bt_metrics.get('message', 'Unknown error') if st.session_state.bt_metrics else 'Unknown error'}")
