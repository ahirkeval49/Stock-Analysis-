...
def fetch_fair_value_from_value_trades(ticker: str, company_name: str) -> dict:
    ...
    "The fair value of Apple Inc (AAPL) is $179. The current price ($200.85) indicates stock is overvalued by 12.21%."
    """
    if not company_name:
        company_name_for_slug = ticker
    else:
        company_name_for_slug = company_name

    name_part = company_name_for_slug.lower().replace('.', '').replace(',', '').replace(' inc', '').replace(' corp', '').replace(' company', '').replace(' incorporated', '').replace(' limited', '').replace(' ltd', '')
    slug = f"{ticker.lower()}-{'-'.join(name_part.split()[:3])}".replace('--','-')
    stock_page_url = f"https://value-trades.com/stock/{slug}/"
    simple_slug_url = f"https://value-trades.com/stock/{ticker.lower()}/"
    ...
    response_stock_page = None
    final_url_attempted = ""

    try:
        final_url_attempted = stock_page_url
        response_stock_page = session.get(stock_page_url, timeout=15, allow_redirects=True)
        
        if response_stock_page.status_code != 200 or ticker.upper() not in response_stock_page.text[:10000]:
            if stock_page_url != simple_slug_url:
                final_url_attempted = simple_slug_url
                response_stock_page = session.get(simple_slug_url, timeout=15, allow_redirects=True)
                # stock_page_url = simple_slug_url # No need to reassign stock_page_url, final_url_attempted has it
                if response_stock_page.status_code != 200 or ticker.upper() not in response_stock_page.text[:10000]:
                    return {"error": f"VT: Failed to fetch valid stock page for {ticker} with constructed URLs. Status: {response_stock_page.status_code}. URL tried: {final_url_attempted}",
                            "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}
            else: 
                 return {"error": f"VT: Failed to fetch valid stock page for {ticker}. Status: {response_stock_page.status_code}. URL tried: {final_url_attempted}",
                            "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}
        ...
        for tag in tags_to_search:
            ...
            if match:
                ...
                try:
                    ...
                    site_current_price = float(match.group(2).replace(',', ''))
                    valuation_status = match.group(3).lower()
                    valuation_percentage = float(match.group(4).replace(',', ''))
                    found_sentence = True
                    break 
                except ValueError as ve:
                    return {"error": f"VT: Found valuation sentence for {ticker} but failed to parse numbers: '{full_valuation_text}'. Error: {ve}", 
        ...
        if found_sentence:
            ...
            return {"error": f"VT: Specific fair value sentence not found for {ticker} on page {final_url_attempted}.", 
                    "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}

    except requests.exceptions.Timeout:
        return {"error": f"VT: Timeout accessing {final_url_attempted or initial_search_url}", "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}
    except requests.exceptions.HTTPError as http_err:
        return {"error": f"VT HTTP error for {ticker} ({http_err.response.status_code}) on URL {final_url_attempted or initial_search_url}", 
                "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}
        ...
        return {"error": f"VT Unexpected general error for {ticker}: {e}", "vt_fair_value": None, "vt_current_price": None, "vt_valuation_text": None}
    finally:
...
class ModelClient:
    ...
    def embed(self, texts: list[str], model_id: str = "text-embedding-ada-002") -> list[list[float]]:
        ...
        #     actual_embedding_model = "deepseek-embedder" 
        try:
...
class FairValueAgentVT:
    def run(self, ticker: str, data: dict) -> dict:
        ...
        error_specific_sentence_prefix = f"VT: Specific fair value sentence not found for {ticker} on page" # Prefix for matching

        significant_error = False
        if error:
            # Check if the error is one of the "expected" non-finding errors or config issues
            is_ignorable_error = (
                error == error_fv_not_found_page or
                error.startswith(error_specific_sentence_prefix) or # Use startswith for the URL variant
                error == "VT Configuration incomplete in secrets." or
                error == "VT: Skipped by user config."
            )
...
def run_live_analysis(tickers, history_years, llm_client, configs):
    ...
    for t in tickers:
        ...
        company_name_for_vt = ticker_info.get('longName', ticker_info.get('shortName', t)) # Get company name for VT scraper
        ...
        data_bundle = {
            ...
            "value_trades_fair_value_data": fetch_fair_value_from_value_trades(t, company_name_for_vt) if configs["use_value_trades"] else \
                    ...
                    with tabs[2]:
                        ...
                        if live_configs_main["use_value_trades"]:
                            ...
                            if res.get('vt_fair_value_estimate') is None and not res.get('vt_data_error'):
                                ...
                                vt_scrape_status = "Not Attempted / Config Error" # Corrected this part for clarity
